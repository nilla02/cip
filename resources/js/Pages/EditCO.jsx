...
export default function UpdateProfileInformation({ users,auth,submission}) {
    const { data, setData, put, errors, processing } = useForm({
        ...
        Assigned_Compliance:submission.Assigned_Compliance,
    });
    ...
    const updatePassword = (e) => {
        ...
        put(route("EditCO.assignco", submission.id), {
            preserveScroll: true,
            onSuccess: () => reset(),
            onError: (errors) => {
                console.log(errors);
        ...
        >
            ...
            <header>
               ...
               Officer Assignment Form
                ...
                Assign which Compliance officer will be linked to this application
                </p>
            </header>
    ...
    <select
    onChange={(e) => setData('Assigned_Compliance', e.target.value)}
    className="mt-1 block w-full"
    value={data.Assigned_Compliance}
>
<option value="">-Select Assigned Due Diligence Officer-</option>
    ...
    <InputError message={errors['Assigned_Compliance']} className="mt-2" />
</div>
